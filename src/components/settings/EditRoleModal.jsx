...
import { X, Shield, Save, AlertCircle, Trash2 } from 'lucide-react';
import { useSettings } from '../../contexts/SettingsContext';

const EditRoleModal = ({ isOpen, onClose, role, onSuccess, onDelete }) => {
  const { updateRole, fetchPermissions, permissions: permissionsData } = useSettings();
  ...
  const handleSubmit = async (e) => {
    ...
    if (role?.is_system_role && formData.name !== role.name) {
      setError('Cannot rename system roles');
      return;
    }
    ...
    try {
      await updateRole(role.id, {
        name: role.is_system_role ? role.name : formData.name.trim(), // Don't allow renaming system roles
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-indigo-100 text-sm">
                {role.is_system_role ? 'System Role (Name cannot be changed)' : 'Modify role details and permissions'}
              </p>
            </div>
          </div>
          <button
          ...
          {role.user_count > 0 && (
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-2">
              ...
              <div className="text-sm text-yellow-800">
                <span className="font-medium">{role.user_count} user(s)</span> currently have this role.
                Changes to permissions will affect them immediately.
              </div>
            </div>
          )}
                  ...
                  <div key={moduleName} className="border-b border-gray-200 last:border-b-0">
                    ...
                    <div className="bg-gray-50 px-4 py-3 flex items-center justify-between sticky top-0">
                      ...
                      {someSelected && !allSelected && (
                        <span className="text-xs text-indigo-600 font-medium">Partial</span>
                      )}
                    </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-200">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-6 py-2 bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg hover:from-indigo-700 hover:to-purple-800 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Save size={18} />
