    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-lift w-full max-w-lg">
        <div className="px-5 py-4 border-b border-ink-100">
          <h2 className="text-lg font-semibold text-ink-900">Request replenishment</h2>
        ...
        <form onSubmit={submit} className="px-5 py-4 space-y-3 text-sm">
          <div className="bg-ink-50 border border-ink-100 rounded-md p-3">
            <div className="text-xs uppercase text-ink-500">Top-up amount</div>
            <div className="text-xl font-semibold text-ink-900">
              {pettyAccount.currency} {expectedRequest.toLocaleString()}
            </div>
            <div className="text-xs text-ink-500 mt-1">imprestLimit − currentBalance</div>
          </div>

          <label className="block">
            <span className="font-medium text-ink-700">Source account (optional — set later if blank)</span>
            <select
