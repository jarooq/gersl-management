    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
        <div className="px-5 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Transfer cash</h2>
          <p className="text-xs text-gray-500 mt-1">From {fromAccount.name} ({fromAccount.currency} {Number(fromAccount.currentBalance).toLocaleString()})</p>
        </div>
        <form onSubmit={submit} className="px-5 py-4 space-y-3 text-sm">
          ...
          <label className="block">
            <span className="font-medium text-gray-700">Amount ({fromAccount.currency})</span>
            <input
              type="number"
              step="0.01"
              min={0}
