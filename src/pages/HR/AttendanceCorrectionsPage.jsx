...
export default function AttendanceCorrectionsPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-4">
      ...
      {showForm && (
        <form onSubmit={onSubmitRequest} className="bg-white border border-ink-100 rounded-md p-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
          <label className="block">
            <span className="text-xs uppercase text-ink-500">Staff ID</span>
            <input
              type="number"
              value={form.staffId}
          ...
          <label className="block">
            <span className="text-xs uppercase text-ink-500">Attendance row ID (blank for missing-day)</span>
            <input
              type="number"
          ...
          <label className="block">
            <span className="text-xs uppercase text-ink-500">Date</span>
            <input
              type="date"
          ...
          <label className="block">
            <span className="text-xs uppercase text-ink-500">Field</span>
            <select
              value={form.field}
          ...
          <label className="block sm:col-span-2">
            <span className="text-xs uppercase text-ink-500">New value{form.field === 'newRecord' && ' (JSON: {status, checkInTime, ...})'}</span>
            <input
              value={form.newValue}
          ...
          <label className="block sm:col-span-3">
            <span className="text-xs uppercase text-ink-500">Reason</span>
            <textarea
              rows={2}
              value={form.reason}
          ...
          <div className="sm:col-span-3 flex justify-end gap-2">
            <button type="button" onClick={() => setShowForm(false)} className="px-3 py-1.5 text-sm border border-ink-200 rounded-md">Cancel</button>
            <button type="submit" disabled={busy} className="px-3 py-1.5 text-sm font-medium text-white bg-navy-900 rounded-md hover:bg-navy-800 shadow-card transition disabled:opacity-50">
      ...
      <section className="bg-white border border-ink-100 rounded-md p-4 flex flex-wrap items-end gap-3 text-sm">
        <div className="font-medium text-ink-700">Register reports</div>
        <label>
          <span className="text-xs uppercase text-ink-500 block">From</span>
          <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="rounded-md border border-ink-200 px-2 py-1" />
        </label>
        <label>
          <span className="text-xs uppercase text-ink-500 block">To</span>
          <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="rounded-md border border-ink-200 px-2 py-1" />
        </label>
        <a
      ...
      <div className="flex gap-2 border-b border-ink-100">
        ...
        {isHR && (
          <>
            ...
            <button
              onClick={() => setTab('all')}
              className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px ${tab === 'all' ? 'border-blue-600 text-blue-600' : 'border-transparent text-ink-500 hover:text-ink-700'}`}
            >All</button>
          </>
        )}
      </div>
        ...
        <table className="min-w-full divide-y divide-ink-100">
          <thead className="bg-ink-50">
            <tr>
              ...
              <th className="px-3 py-2 text-left text-xs font-medium text-ink-500 uppercase">Field</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-ink-500 uppercase">Old → New</th>
              <th className="px-3 py-2 text-left text-xs font-medium text-ink-500 uppercase">Reason</th>
          ...
          <tbody className="bg-white divide-y divide-ink-100">
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={7} className="p-6 text-center text-sm text-ink-500">No corrections.</td></tr>
            )}
            {rows.map(c => (
              <tr key={c.id}>
                <td className="px-3 py-2 text-sm text-ink-700">{c.attendanceDate}</td>
                <td className="px-3 py-2 text-sm">
                  <div className="text-ink-900">Staff #{c.staffId}</div>
                  <div className="text-xs text-ink-500">by {c.requester?.fullName || '—'}</div>
                </td>
                <td className="px-3 py-2 text-sm">{c.field}</td>
                <td className="px-3 py-2 text-sm">
                  <span className="text-ink-500">{c.oldValue || '—'}</span>
                  <span className="mx-1 text-ink-400">→</span>
                  <span className="text-ink-900">{c.newValue || '—'}</span>
                </td>
                <td className="px-3 py-2 text-sm text-ink-700">{c.reason}</td>
                ...
                <td className="px-3 py-2 text-sm space-x-2 whitespace-nowrap">
                  ...
                  {c.status === 'Pending' && c.requestedBy === user?.id && (
                    <button disabled={busy} onClick={() => action(() => AttendanceCorrectionAPI.cancel(c.id))} className="text-ink-500 hover:underline">Cancel</button>
                  )}
                </td>
              </tr>
            ))}
