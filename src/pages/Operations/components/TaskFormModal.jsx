...
const TaskFormModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (mode === 'edit' && task) {
        ...
        // Parse media coverage type and media team assigned
        const parsedMediaCoverageType = (() => {
        ...
        const parsedMediaTeamAssigned = (() => {
          ...
          if (typeof task.mediaTeamAssigned === 'string') {
            try {
              ...
              return task.mediaTeamAssigned ? [task.mediaTeamAssigned] : [];
            }
          }
          return [];
        })();
        ...
        setFormData({
          ...
          requiresProcurement: task.requiresProcurement || false,
          procurementItems: task.procurementItems || '',
          estimatedBudget: task.estimatedBudget || '',
          involvesBeneficiaries: task.involvesBeneficiaries || false,
          beneficiaryTrackingMode: task.beneficiaryTrackingMode || 'None',
          beneficiarySelectionMethod: task.beneficiarySelectionMethod || '',
          selectionCriteria: task.selectionCriteria || '',
          targetBeneficiaries: task.targetBeneficiaries || '',
          distributionType: task.distributionType || 'None',
          distributionItems: task.distributionItems || '',
          cashAmount: task.cashAmount || '',
          distributionDate: formatDate(task.distributionDate),
          distributionLocation: task.distributionLocation || '',
          trainingType: task.trainingType || '',
          trainingDuration: task.trainingDuration || '',
          certificateProvided: task.certificateProvided || false,
          mediaCoverageRequired: task.mediaCoverageRequired || false,
          mediaCoverageType: parsedMediaCoverageType,
          mediaTeamAssigned: parsedMediaTeamAssigned
        });
      } else {
  ...
  const handleTaskTypeChange = (taskType) => {
    setFormData(prev => ({
      ...prev,
      taskType,
      // Auto-set flags based on task type (only in create mode)
      ...(mode === 'create' ? {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'edit') {
        console.log('✅ Updating task:', task.id, taskData.title);
      ...
      setErrors({ submit: error.message || `Failed to ${mode === 'edit' ? 'update' : 'create'} task. Please try again.` });
    } finally {
      setLoading(false);
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Ready to {mode === 'edit' ? 'update' : 'create'}!</strong> Review your task details and click {mode === 'edit' ? 'Update Task' : 'Create Task'} to finalize.
                </p>
              </div>
            </div>
          )}
